  ...
  wrapper <- function(x, y, z) {
    outer <- function(d, e, f) {
      inner <- function(g, h, i) {
        ...
        exec_context(max_calls = 30, call_subset = c(knitter_offset, -1))
      }
...
The evaluated exec_context would give you a structure with these 3 fields:
...
new_log_layout(
  format = list(
    ...
    new_fmt_log_msg(),
    new_fmt_line_break(),
    new_fmt_exec_scope(crayon::bgMagenta$bold, 'call_stack')
  ),
  seperator = ' ',
