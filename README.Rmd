...
are covered in detail in the *LogDispatch* section of the manual.
...
All levels that have been specified via the config can be found by calling *log_levels*:
...
And you can get detailed information about a particular log level via *level_info()*:

```{r}
level_info(log_levels("warn"))
```

You'll notice that a level has **level** and **message** style attributes that are [crayon](https://github.com/r-lib/crayon) styles. This is because each level has a unique render format that helps to visually assist parsing of log information.

To see the sample format of all loaded log levels, call *display_log_levels*:

```{r}
display_log_levels()
```

The default logging configuration closely resembles the fairly ubiquitous [log4j](https://logging.apache.org/log4j/1.2/apidocs/org/apache/log4j/Level.html) scheme.
...
The default log layout is a standard format: {LEVEL} - {TIMESTAMP} - {MSG}, with space as a separator between format objects and
the new line feed *"\n"*.
...
## Acknowledgments
  ...
  * The pretty logger output in this document is made possible by the excellent [fansi](https://github.com/brodieG/fansi) package by [Brody Gaslam](https://twitter.com/BrodieGaslam).
